...
class NoslipDomain : public SubDomain
{
  ...
  {
    return (on_boundary &&
            (x[0] < DOLFIN_EPS || x[1] < DOLFIN_EPS || x[0] > 1.0 - DOLFIN_EPS || x[1] > 1.0 - DOLFIN_EPS 
            || (pow(x[0] - 0.5, 2) + pow(x[1] - 0.5, 2) <= pow(0.25 + DOLFIN_EPS, 2))));
  }
};
...
int main()
{
  ...
  // Load mesh from file
  auto mesh = std::make_shared<Mesh>("../mesh/kek.xml");
